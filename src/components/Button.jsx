...
export default function Button({ children, to, href, variant = 'primary', onClick, type = 'button' }) {
  ...
  if (to) return <><Link to={to} className={className} data-cursor="link">{content}</Link>{styleTag}</>;
  if (href) return <><a href={href} className={className} data-cursor="link">{content}</a>{styleTag}</>;
  return <><button type={type} onClick={onClick} className={className} data-cursor="link">{content}</button>{styleTag}</>;
}
